...
(function () {
    ...
    console.log(chalker.yellow.bold(`Initializing Database Connection for DRY - RUN .............`));

    const MongooseInstance = require("mongoose").Mongoose;
    const mongoConnectOptions = { "useNewUrlParser": true, socketTimeoutMS: 45000000 };

    let result = [];

    async.waterfall([
        _selectReadOp(result),
        _selectWriteOp
    ], function (err, result) {
        ...
        var readOption = result[0];
        var writeOption = result[1];

        var readUrl = null;
        var writeUrl = null;

        if (readOption && readOption >= 1 && readOption <= 5) {
            readUrl = dbURLs[readOption - 1];
            if (readUrl) {

                const readInstance = new MongooseInstance();
                readInstance.connect(readUrl, mongoConnectOptions);
                db = readInstance.connection;
        ...
        if (writeOption && writeOption >= 1 && writeOption <= 5) {
            writeUrl = dbURLs[writeOption - 1];
            if (writeUrl) {
                const writeInstance = new MongooseInstance();
                writeInstance.connect(writeUrl, mongoConnectOptions);
                localDb = writeInstance.connection;
            } else {
                console.log(chalker.red.bold(`Invalid option entered .............`));
                process.exit(1);
            }
        ...
        var readInstancePromise = new Promise((resolve, reject) => {

            db.on('error', function () {
                console.log("Read Connection Error.... for url ", readUrl);
                reject();
            });

            db.once('open', function callback() {
                console.log("Read Connection established to : ", readUrl);
                resolve();
            });
        });

        var writeInstancePromise = new Promise((resolve, reject) => {

            localDb.on('error', function () {
                console.log("Write Connection Error.... for url ", writeUrl);
                reject();
            });

            localDb.once('open', function callback() {
                console.log("Write Connection established to : ", writeUrl);
                resolve();
            });
        ...
        Promise.all([readInstancePromise, writeInstancePromise]).then(() => {
            run();
        }).catch(e => {
            console.error(`Error on multiple instance connection : `, e);
            process.exit(1);
        });
    });

    function _selectReadOp(result) {
        return function (callback) {
            chooseEnvironment(`Choose option to connect to database for read operation :  DRY - RUN`).then(readOption => {
                result.push(readOption);
                callback(null, result);
            });
        }
    }

    function _selectWriteOp(result, callback) {
        chooseEnvironment(`Choose option to connect to database for write operation :  DRY - RUN`).then(writeOption => {
            result.push(writeOption);
            callback(null, result);
        });
    }
...
function chooseEnvironment(msg) {
    return new Promise((resolve, reject) => {
        rl.question(
            `\n
            #. ${msg ? msg : `Choose option to connect to database :  DRY - RUN`}
...
function _populateData(params, callback) {
    ...
    const queue = async.queue(function (data, queueCB) {
        ...
        if (franchise) {
            ...
            getIdGenerator(localDb, 'RSA', 'retailerShemeAchievedId', false, count).then(result => {
    ...
    queue.drain = function () {
        if (isDbOp === false && count > 0) {
            localDb.collection('counters').findOneAndUpdate({ _id: 'retailerShemeAchievedId' }, { $set: { next: count } }, { new: true }, function (err, result) {
                if (err) {
                    console.log(chalker.red.bold(`# SEVER ERROR - Counters not updated , current count = ${count}`));
                }
            });
        }
        callback(null, params);
...
function _insertRecords(params, callback) {
    const bulk = localDb.collection('retailerschemeachieveds').initializeOrderedBulkOp();

    params.insertionList.map(record => {
        bulk.find({ _id: record._id }).upsert().updateOne(record);
    })
...
var getCount = function (refDB, sequenceName, expire, callback) {
    var options = {};
    if (!expire) {
    ...
    refDB.collection('counters').findOneAndUpdate({ _id: sequenceName }, { $inc: { next: 1 }, $set: { expiresAt: expire } }, options, callback);
};

function getIdGenerator(refDB, prefix, counterName, isDbOp = true, count = 0) {
    return new Promise((resolve, reject) => {
        if (isDbOp === false && count === 0) {
            getCount(refDB, counterName, null, function (err, doc) {
                let _id = prefix + doc.value.next;
                resolve({ _id: _id, count: doc.value.next });
            });
        } else if (isDbOp === false && count > 0) {
            count++;
            let _id = prefix + count;
            resolve({ _id: _id, count: count });
